...
- Configure checkpoint now copies symlinks as symlinks (Flutter iOS
  `ephemeral/Packages/.packages/*` links no longer fail with
  `OS Error: Is a directory`)
- Skip Flutter `ephemeral` directories in checkpoints (regenerated by Flutter)
